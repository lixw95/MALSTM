...
class Decoder(nn.Module):
    def __init__(self, T, decoder_num_hidden, encoder_num_hidden):
        ...
        self.attn_layer = nn.Sequential(nn.Linear(self.decoder_num_hidden * 3, decoder_num_hidden), nn.Linear(encoder_num_hidden, 1))
        ...
        self.fc = nn.Linear(self.decoder_num_hidden + 1, 1)
        self.fc_final = nn.Linear(self.decoder_num_hidden * 2, 1)
...
class DA_RNN(nn.Module):
    ...
    def train(self):
        ...
        self.X, self.y =self.load_train_dataset(debug=False)
        n_iter = 0
    ...
    def load_train_dataset(self, debug=True):
        main_path = r"/home/lxw/1-th_code/MALSTM/DA-RNN-master/contrast_experiment/nasdaq100_padding.csv"
        df = pd.read_csv(main_path,nrows=11200 if debug else None)
    ...
    def test(self, on_train=False):
        ...
        plt.savefig('D:/download_code/DA-RNN-attchange1/DA-RNN-master/contrast_experiment/without_spatialattn.png')
        plt.close(fig3)


if __name__ == '__main__':
    print("==> Load dataset ...")
    model = DA_RNN(12, 128, 128, 52, 0.001, 800)
    model.train()
    torch.save(model, 'D:/download_code/DA-RNN-attchange1/DA-RNN-master/contrast_experiment/without_spatialattn.pth')
    #model = torch.load('D:/download_code/DA-RNN-attchange1/DA-RNN-master/contrast_experiment/without_spatialattn.pth')
    model.test()