...
class Encoder(nn.Module):
    def __init__(self, T, input_size, encoder_num_hidden, parallel=False):
        ...
        self.encoder_attn = nn.Linear(in_features=self.encoder_num_hidden + self.T - 1, out_features=1)
        self.encoder_attn_v = nn.Linear(in_features=self.encoder_num_hidden + 1, out_features=1)
    ...
    def forward(self, X, y_prev):
        ...
        for t in range(self.T - 1):
            x = torch.cat((h_n1.repeat(self.input_size, 1, 1).permute(1, 0, 2), x_y.permute(0, 2, 1)), dim=2)
            x = self.encoder_attn(x.view(-1, self.encoder_num_hidden + self.T - 1))
            alpha = F.softmax(x.view(-1, self.input_size), dim=1)
            x_tilde = torch.mul(alpha, x_y[:, t, :])
            self.encoder_gru.flatten_parameters()
            _, final_state = self.encoder_gru(x_tilde.unsqueeze(0), h_n1)
            h_n1 = final_state[0]
            h_n1 = h_n1.unsqueeze(0)
            #s_n1 = final_state[1]
            X_tilde[:, t, :] = x_tilde
            y = torch.cat((h_n2.repeat(self.input_size1, 1, 1).permute(1, 0, 2), mm.permute(0, 2, 1)), dim=2)
            y = self.encoder_attn(y.view(-1, self.encoder_num_hidden + self.T - 1))
            alpha1 = F.softmax(y.view(-1, self.input_size1), dim=1)
            x_tilde1 = torch.mul(alpha1, mm[:, t, :])
            self.encoder_gru1.flatten_parameters()
            _, final_state = self.encoder_gru1(x_tilde1.unsqueeze(0), h_n2)
            h_n2 = final_state[0]
            h_n2 = h_n2.unsqueeze(0)
            #s_n2 = final_state[1]
...
class Decoder(nn.Module):
    def __init__(self, T, decoder_num_hidden, encoder_num_hidden):
        ...
        self.attn_layer = nn.Sequential(nn.Linear(self.decoder_num_hidden * 4, decoder_num_hidden), nn.Linear(encoder_num_hidden, 1))
    ...
    def forward(self, X_encoded, y_prev):
        ...
        for t in range(self.T - 1):
            x = torch.cat((d_n.repeat(self.T - 1, 1, 1).permute(1, 0, 2), X_encoded), dim=2)
            beta = F.softmax(self.attn_layer(x.view(-1, x.shape[2])).view(-1, self.T - 1), dim=1)
            context_x = torch.bmm(beta.unsqueeze(1), X_encoded)[:, 0, :]
            if t < self.T - 1:
                y = torch.cat((context_x, y_prev[:, t].unsqueeze(1)), dim=1)
                y_tilde = self.fc(y)
                self.gru_layer.flatten_parameters()
                _, final_states = self.gru_layer(y_tilde.unsqueeze(0), d_n)
                d_n = final_states[0]
                d_n = d_n.unsqueeze(0)
                #c_n = final_states[1]
        y_pred = self.fc_final(torch.cat((d_n[0], context_x), dim=1))
        return y_pred
    def _init_states(self, X):
...
class DA_RNN(nn.Module):
    def __init__(self, T, encoder_num_hidden, decoder_num_hidden, batch_size, learning_rate, epochs, parallel=False):
        ...
        self.Decoder = Decoder(encoder_num_hidden=encoder_num_hidden, decoder_num_hidden=decoder_num_hidden, T=T).to(self.device)
    ...
    def load_train_dataset(self, year):
        main_path = r"D:/download_code/DA-RNN-attchange1/DA-RNN-master/train_set220_knn"
    ...
    def test(self, on_train=False):
        ...
        for k in range(len(test_num)):
            ...
            error_sum[k] = error
        rmse_arr = np.mean(rmse_sum)
        print('RMSE', rmse_arr)
        error_s = np.mean(error_sum)
        print('average error：', error_s)
        var = np.var(error_sum, axis=0)
        print('variance:', var)
        fig3 = plt.figure()
...
if __name__ == '__main__':
    print("==> Load dataset ...")
    model = DA_RNN(12, 128, 128, 52, 0.001, 1200)
    model.train()
    torch.save(model, 'D:/download_code/DA-RNN-attchange1/DA-RNN-master/src/save_model/gru.pth')
    #model = torch.load('D:/download_code/DA-RNN-attchange1/DA-RNN-master/src/save_model/model5.pth')
    model.test()