...
class Encoder(nn.Module):
    ...
    def forward(self, X):
        ...
        for t in range(self.T - 1):
            x = torch.cat((h_n1.repeat(self.input_size, 1, 1).permute(1, 0, 2),s_n1.repeat(self.input_size, 1, 1).permute(1, 0, 2), X.permute(0, 2, 1)), dim=2)
            ...
            s_n1 = final_state[1]
            X_tilde[:, t, :] = x_tilde
            y = torch.cat((h_n2.repeat(self.input_size1, 1, 1).permute(1, 0, 2),s_n2.repeat(self.input_size1, 1, 1).permute(1, 0, 2), X.permute(0, 2, 1)), dim=2)
            y = self.encoder_attn(y.view(-1, 2 * self.encoder_num_hidden + self.T - 1))
            alpha1 = F.softmax(y.view(-1, self.input_size1), dim=1)
            x_tilde1 = torch.mul(alpha1, X[:, t, :])
...
class Decoder(nn.Module):
    def __init__(self, T, decoder_num_hidden, encoder_num_hidden):
        ...
        self.lstm_layer = nn.LSTM(input_size=1, hidden_size=self.encoder_num_hidden * 2, num_layers=1)
        self.gru_layer = nn.GRU(input_size=1, hidden_size=encoder_num_hidden + decoder_num_hidden)
        self.fc = nn.Linear(self.encoder_num_hidden * 2 + 1, 1)
        self.fc_final = nn.Linear(self.encoder_num_hidden * 2, 1)
...
class DA_RNN(nn.Module):
    ...
    def load_test_dataset(self, year):
        main_path = r"D:/download_code/DA-RNN-attchange1/DA-RNN-master/test_data"
        file_name = '{}.csv'.format(year)
        file_path = os.path.join(main_path, file_name)
        df = pd.read_csv(file_path)
        data = df.iloc[:, 1:].values
        ...
        for k in range(len(test_num)):
            ...
            while i < len(y_pred):
                ...
                _, input_encoded = self.Encoder(
                    Variable(torch.from_numpy(X).type(torch.FloatTensor).to(self.device)), y_history)
...
if __name__ == '__main__':
    ...
    model.train()
    torch.save(model, 'D:/download_code/DA-RNN-attchange1/DA-RNN-master/src/save_model/modelunadd_y.pth')
